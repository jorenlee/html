...
onMounted(() => {
  ...
  {
    router.push("/human-resource/dashboard/portal/create");
  } else {
...
const uploadedFile = (e) => {
  ...
  info.value.image_link = e.target.files[0].name;
}
...
const saveFile = async () => {
  ...
  axios
    .post(endpoint.value + "/api/humanResource/file/list/files/", formData, axiosConfig)
    .then((response) => {
...
const submitFunc = async () => {
  ...
  .then((response) => {
    ...
    router.push({ path: "/human-resource/dashboard/portal/lists" });
  });
}
      ...
      >
        <div>
          ...
          <div class="mx-auto mt-10 mb-5 grid grid-cols-1">
            <a
              href="/human-resource/dashboard/portal/lists"
            ...
            <a
              href="/human-resource/dashboard/portal/create"
              class="text-xs mx-auto mb-2 w-full uppercase whitespace-nowrap px-5 py-1 font-bold text-left text-green-900 hover:bg-green-900 hover:text-white"
            >
          ...
          <div class="w-full p-5">
            <form v-on:submit.prevent="submitForm" class="px-3 flex flex-wrap">
              ...
              <div class="w-fit mt-5">
                <button class="w-fit mx-auto block bg-green-800 px-8 py-1.5 rounded-lg text-white">Submit</button>
              </div>
            </form>
          </div>
        </div>
