...
const careers = await $fetch(endpoint.value + "/api/humanResource/list").catch((error) => error.data);
// console.log(careers)
...
onMounted(() => {
  if (
    ...
    router.push("/human-resource/dashboard/portal/lists");
  } else {
...
<div>
    ...
    <div class="pt-7 pb-24 w-3/12 px-5 bg-gray-100 lg:block hidden" v-show="toggleSideBarMenu">
      <div>
        ...
        <div class="mx-auto mt-10 mb-5 grid grid-cols-1">
          <a href="/human-resource/dashboard/portal/lists"
          ...
          <a href="/human-resource/dashboard/portal/create"
            class="text-xs mx-auto mb-2 w-full uppercase whitespace-nowrap px-5 py-1 font-bold text-left text-green-900 hover:bg-green-900 hover:text-white">
        ...
        <div>
          <div class="w-11/12 mx-auto shadow lg:px-5 px-4 lg:pt-5 pt-3 my-10 rounded-lg">
            ...
            <div class="w-full rows" v-if="careers.length > 0">
              <ul class="lg:grid grid-cols-5 gap-5 lg:py-10">
                <li v-for="(c, i) in careers" :key="i"
                  class="border p-3 rounded-lg hover:rounded-none shadow-md hover:shadow-none lg:my-0 my-5 bg-white hover:bg-green-800">
                  <img class="lg:mx-0 mx-auto w-[500px] h-[200px]"
                    :src="`https://lsu-media-styles.sgp1.digitaloceanspaces.com/storage-bucket-name/folder-location/files/humanResource/files/${c.image_link}`" />
                  <!-- :src="`http://127.0.0.1:8000/media/files/humanResource/files/${c.image_link}`" -->
            ...
            <div v-else class="itallic text-gray-400 text-center p-10">
  ...
  <div class="bg-green-900 py-4">
    <h1 class="text-center text-white lg:text-sm text-xs">
      Copyright © 2023 <span>La Salle University Ozamiz</span>
    </h1>
  </div>
</div>
</template>
