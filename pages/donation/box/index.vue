<script setup>
  ...
  const imagePreview = ref("https://lsu-media-styles.sgp1.digitaloceanspaces.com/storage-bucket-name/folder-location/files/admissions/sample.jpg")
  ...
  const saveFile = () => {
    ...
    axios
    .post(imageUploaderEndpoint.value + "/api/investiture/file/list/files/", formData, axiosConfig)
    .then((response) => {
...
<template>
 ...
 <!-- accept="image/png, image/jpeg ;capture=camera"  -->
    ...
    <div class="mt-3 mb-14 lg:w-fit mx-auto bg-green-800 shadow" v-if="imagePreview">
        ...
        <a href="mailto:dev@lsu.edu.ph" class="text-xs text-center font-light block">
            dev@lsu.edu.ph
        </a>
        </div>
    </div>
    ...
    <div class="my-5 w-11/12 mx-auto md:w-5/12 shadow-lg bg-white pb-5 lg:pt-0 pt-5 border-t border-gray-100" v-if="receiptBox">
        ...
        <div v-if="thankYouMessage" class="lg:h-screen flex items-center">
          ...
          <a href="/donation" class="hover:text-green-800 font-montserrat text-sm mt-10 uppercase hover:font-bold hover:bg-white bg-green-900 border border-green-800 text-white cursor-pointer w-fit mx-auto px-10 py-1 block select-none rounded-3xl"> back to main page </a>
        </div>
