...
onMounted(() => {
  if (userStore.user.isAuthenticated &&
    ...
    router.push("/_/panels/procurement/edit/" + route.params.id);
  } else {
...
const saveFile = async () => {
  ...
  axios
  .post(endpoint.value + "/api/procurements/file/list/files/", formData, axiosConfig)
  .then((response) => {
    // console.log(response);
    router.push({ path: "/_/panels/procurement" });
  });
}
...
const goToList = () => {
  router.push("/_/panels/procurement");
}

const goToCreate = () => {
  router.push("/_/panels/procurement/create");
}
