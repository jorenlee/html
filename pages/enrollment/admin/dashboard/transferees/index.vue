<script setup>
  ...
  let filterSearch = async () => {
    if (searchText.value === '') {
      ...
      startNum.value = 0  
      enrolleesList.value = await $fetch(endpoint.value + "/api/admissions-first-sem/transferee/list/").catch((error) => error.data);
  ...
  onMounted(async () => {
    ...
    let currentList = await $fetch(endpoint.value + "/api/admissions-first-sem/transferee/list/").catch((error) => error.data);
    ...
    router.push("/enrollment/admin/dashboard/transferees");
  ...
  const deleteItem = async () => {
    await $fetch(
      endpoint.value + "/api/admissions-first-sem/transferee/" + idToBeDeleted.value + "/delete/",
      {
        method: "DELETE",
        headers: {
  ...
  const directDelete = async (id) => {
    await $fetch(endpoint.value + "/api/admissions-first-sem/transferee/" + id + "/delete/", {
      method: "DELETE",
      headers: {
        Authorization: userStore.user.token,
        "Content-Type": "application/json",
  ...
  const displaySingleEntry = async (id) => {
    singleEnrollee.value = await $fetch(endpoint.value + "/api/admissions-first-sem/transferee/" + id + "/").catch((error) => error.data)
  ...
  const editStatus = async () => {
    await $fetch(endpoint.value + "/api/admissions-first-sem/transferee/" + singleEnrollee.value.id + "/edit/", {
      method: "PUT",
      body: singleEnrollee.value,
    })
    .then(async () => {
      spinner.value = true;
      enrolleesList.value = await $fetch(endpoint.value + "/api/admissions-first-sem/transferee/list/")
      .then((response) => {
