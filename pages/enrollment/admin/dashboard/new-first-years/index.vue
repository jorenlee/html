<script setup>
  ...
  let filterSearch = async() => {
    if (searchText.value === '') {
      ...
      startNum.value = 0  
      enrolleesList.value = await $fetch(endpoint.value + "/api/admissions-first-sem/new-student/list/").catch((error) => error.data);
  ...
  onMounted(async () => {
    ...
    let currentList = await $fetch(endpoint.value + "/api/admissions-first-sem/new-student/list/").catch((error) => error.data);
    ...
    router.push("/enrollment/admin/dashboard/new-first-years");
  ...
  // const defaultRemarksAdmissions = ref([
  //   'Please proceed to CAS at BVM 215 for course advising. Thank you.', 
  //   'Please proceed to CCJE at BVM 215 for course advising. Thank you.',   
  //   'Please proceed to CCSEA at BVM 215 for course advising. Thank you.',
  //   'Please proceed to CON at BVM 215 for course advising. Thank you.',
  //   'Please proceed to CBA at BVM 214 for course advising. Thank you.',
  //   'Please proceed to CTHM at BVM 212 for course advising. Thank you.',
  //   'Please proceed to CTE at BVM 212 for course advising. Thank you.',
  // ])
  ...
  const deleteItem = async () => {
    await $fetch(
      endpoint.value + "/api/admissions-first-sem/new-student/" + idToBeDeleted.value + "/delete/",
      {
        method: "DELETE",
        headers: {
    ...
    .then(async (response) => {
      ...
      toggleConfirmDelete.value = !toggleConfirmDelete.value;
      router.go();
      // enrolleesList.value = await $fetch(endpoint.value + "/api/admissions-first-sem/new-student/list/")
      // .then((response) => {
  ...
  const directDelete = async (id) => {
    await $fetch(endpoint.value + "/api/admissions-first-sem/new-student/" + id + "/delete/", {
      method: "DELETE",
      headers: {
        Authorization: userStore.user.token,
        "Content-Type": "application/json",
  ...
  const displaySingleEntry = async (id) => {
    singleEnrollee.value = await $fetch(endpoint.value + "/api/admissions-first-sem/new-student/" + id + "/").catch((error) => error.data)

    // displaySingleEntryStudentProfile()

    courseSelection.value = singleEnrollee.program
  ...
  const displaySingleEntryStudentProfile = async () => {
    let toFilterStudentProfile = await $fetch(endpoint.value + "/api/admissions-first-sem/student-profile/list/")

    let selectedStudentProfile = toFilterStudentProfile.filter(function (params) {
      return singleEnrollee.value.tracking_id === params.primary_info.tracking_id
    });


    singleEnrolleeStudentProfile.value = selectedStudentProfile


    // console.log('tracking_id', singleEnrollee.value.tracking_id)
    // console.log('to filter', toFilterStudentProfile)
    // console.log('filtered', selectedStudentProfile)
  }

  const editStatus = async () => {
    await $fetch(endpoint.value + "/api/admissions-first-sem/new-student/" + singleEnrollee.value.id + "/edit/", {
      method: "PUT",
...
<!-- 
            ...
            <div class=" w-full shadow-lg px-10 py-5">
              <p class=" flex w-full">
                          ...
                          <p class="flex">
                            <label class="pr-6 w-4/12 text-right"> Program: </label>
                          ...
                          <select v-model="courseSelection" @change="assignCollege"
                              ...
                              <option :value="courseSelection" disabled selected>{{ singleEnrollee.program }}</option>
          ...
          <!-- <div class="bg-green-800 text-white my-3 text-center py-2 uppercase font-bold cursor-pointer" 
            @click="displaySingleEntryStudentProfile">
            Display Student Profile
          </div> -->
         

          <div v-if="singleEnrolleeStudentProfile">
                {{ singleEnrolleeStudentProfile[0].primary_info }}
              </div>
                ...
                <div class="">
                  ...
                  <div class="w-full" v-if="bookingAppointmentListFilter.length > 0">
                    <ul
                      ...
                      v-for="(b, i) in _.orderBy(itemsLoaded, 'created_at', 'desc')" :key="i">
