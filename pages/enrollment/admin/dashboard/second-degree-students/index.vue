<script setup>
  ...
  let filterSearch = async () => {
    if (searchText.value === '') {
      ...
      startNum.value = 0  
      enrolleesList.value = await $fetch(endpoint.value + "/api/admissions-first-sem/second-degree-holder/list/").catch((error) => error.data);
  ...
  onMounted(async () => {
    ...
    let currentList = await $fetch(endpoint.value + "/api/admissions-first-sem/second-degree-holder/list/").catch((error) => error.data);
    ...
    router.push("/enrollment/admin/dashboard/second-degree-students");
  ...
  const deleteItem = async () => {
    await $fetch(
      endpoint.value + "/api/admissions-first-sem/second-degree-holder/" + idToBeDeleted.value + "/delete/",
      {
        method: "DELETE",
        headers: {
  ...
  const directDelete = async (id) => {
    await $fetch(endpoint.value + "/api/admissions-first-sem/second-degree-holder/" + id + "/delete/", {
      method: "DELETE",
      headers: {
        Authorization: userStore.user.token,
        "Content-Type": "application/json",
  ...
  const displaySingleEntry = async (id) => {
    singleEnrollee.value = await $fetch(endpoint.value + "/api/admissions-first-sem/second-degree-holder/" + id + "/").catch((error) => error.data)
  ...
  const editStatus = async () => {
    await $fetch(endpoint.value + "/api/admissions-first-sem/second-degree-holder/" + singleEnrollee.value.id + "/edit/", {
      method: "PUT",
      body: singleEnrollee.value,
    })
    .then(async () => {
      spinner.value = true;
      enrolleesList.value = await $fetch(endpoint.value + "/api/admissions-first-sem/second-degree-holder/list/")
      .then((response) => {
...
<!-- 
          ...
          <div class="w-full px-10 text-center uppercase border-y-4 border-green-900 py-2 tracking-widest font-bold">
            New Second Degree Holders List
          </div>
